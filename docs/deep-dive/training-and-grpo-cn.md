...
!!! note "关于本节中的策略"
    本节介绍了多种可用于开放式答案场景的奖励函数设计策略。**需要注意的是**：
    
    - **项目中的 GRPO 训练示例**（SQL、Calc-X、RAG）都使用**确定性评估**方法（执行结果匹配、数学等价性检查、F1 分数等），这些方法在训练过程中实时计算奖励。
    - **LLM-as-a-Judge** 策略在项目的 **APO（自动提示优化）示例**（`examples/apo/apo_custom_algorithm.py`）中有实际应用，用于在训练过程中评估提示词质量。
    - 本节其他策略（嵌入相似度、多指标组合、人类反馈等）是**推荐的设计方案**，可以根据具体任务需求灵活采用。
...
!!! example "项目实例"
    此策略在 `examples/apo/apo_custom_algorithm.py` 中有实际应用。该示例使用 LLM 在 **APO 训练过程中**评估生成文本的质量。注意这是用于 APO（提示词优化）而非 GRPO 训练。
